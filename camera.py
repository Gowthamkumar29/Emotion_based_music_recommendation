...
# Webcam video stream in separate thread
...
class VideoCamera(object):
    def get_frame(self):
        ...
        for (x, y, w, h) in face_rects:
            ...
            # Resize + normalize + histogram equalization
            resized = cv2.resize(roi_gray_frame, (48, 48))
            equalized = cv2.equalizeHist(resized)
            normalized = equalized / 255.0

            cropped_img = np.expand_dims(np.expand_dims(normalized, -1), 0)
            ...
            cv2.putText(
                image,
                f"{emotion_dict[show_text[0]]} ({confidence*100:.1f}%)",
                (x + 20, y - 60),
